...
(defun soft-get-type-by-id (id)
  (or (aref type-table id)
      (llvm-read-error "Type with ID ~a is not defined" id)))
...
(defparameter next-cst-no nil)

(macrolet ((guard-integer ()
	     `(if (not (eq :integer (car cur-type)))
		  (llvm-read-error "Attempt to read integer constant ~
                                    with non-integer type ~a" cur-type)))
	   (setf0 () `(setf res (list cur-type)))
	   (setf1 () `(setf res (list (car res) cur-type))))
  (define-block constants (:on-undefined-blocks :error) ; :on-undefined-records :error)
    (:around (let ((cur-type nil)
		   (next-cst-no (length value-table)))
	       (let ((it sub-body))
		 (resolve-constant-forward-refs)
		 it)))
    (records ((settype 1) int (:side-effect (setf cur-type (soft-get-type-by-id (car res)))))
	     (null (:side-effect (setf0)))
	     (undef (:side-effect (setf0)))
	     (integer sign-rotated-int (:side-effect (guard-integer) (setf1)))
	     (wide-integer (parse-rest-with-function #'parse-wide-integer)
			   (:side-effect (guard-integer) (setf1)))
	     (float (parse-rest-with-function #'parse-float-constant))
	     (aggregate ...)
	     (string ...)
	     (cstring ...)
	     (ce-binop ...)
	     ...
	     (data ...)
	     (inlineasm ...)
	     (:side-effect (when (not (find (car it) '(:settype) :test #'eq))
			     (assign-value it next-cst-no)
			     (incf next-cst-no)))))
